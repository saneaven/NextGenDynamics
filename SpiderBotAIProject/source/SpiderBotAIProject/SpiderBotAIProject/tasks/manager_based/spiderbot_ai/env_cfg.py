...
class RewardsCfg:
    ...
    reach_target = RewTerm(func=mdp.reach_target_reward, weight=5.0e3)
    death_penalty = RewTerm(func=mdp.death_penalty, weight=-1.0e1)
    feet_ground_time = RewTerm(func=mdp.feet_ground_time_penalty, weight=-5.0e1)
    jump_penalty = RewTerm(func=mdp.jump_penalty, weight=-3.0e1)
    body_angular_velocity = RewTerm(func=mdp.body_angular_velocity_penalty, weight=-1.5e0)
    speed = RewTerm(func=mdp.speed_reward, weight=1.0e3)
    body_vertical_acceleration = RewTerm(func=mdp.body_vertical_acceleration_penalty, weight=-1.0e0)
    ...
    feet_air_time = RewTerm(func=mdp.feet_air_time_reward, weight=5.0e0)
    undesired_contacts = RewTerm(func=mdp.undesired_contacts_penalty, weight=-1.0e1)
    feet_contact_force = RewTerm(func=mdp.feet_contact_force_penalty, weight=-1.0e-7)
    flat_orientation = RewTerm(func=mdp.flat_orientation_l2, weight=-1.0e1)
    wall_proximity = RewTerm(func=mdp.wall_proximity_penalty, weight=-1.0e1)

    patrol_exploration = RewTerm(func=mdp.patrol_exploration_reward, weight=2.0e3)
    patrol_boundary = RewTerm(func=mdp.patrol_boundary_penalty, weight=-2.5e1)
