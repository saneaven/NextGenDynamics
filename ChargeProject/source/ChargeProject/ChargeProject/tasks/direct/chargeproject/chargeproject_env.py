...
class ChargeprojectEnv(DirectRLEnv):
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        counts = (self._last_targets_reached.unsqueeze(-1) >= thresholds).sum(
            dim=0
        ) / self.num_envs
        ...
        horizontal_speed = torch.linalg.norm(
            self._robot.data.root_lin_vel_b[:, :2], dim=1
        )
        ...
        normalized_air_feet = air_feet_per_agent / len(self.feet_ids)
